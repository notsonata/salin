...
- Pass an optional mounted `cookies.txt` file to `yt-dlp`, stage it into a writable temp file, enable a Deno-backed JS challenge runtime, force the Android YouTube client, and let `yt-dlp` choose the available format for current Droplet bot-check recovery
